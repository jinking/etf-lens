...
def sync_shares_cmd(trade_date: str | None = None):
    parsed = date.fromisoformat(trade_date) if trade_date else None
    result = sync_shares(parsed)
    typer.echo(result)


@app.command("backfill-history")
def backfill_history_cmd(
    security_ids: list[str] = typer.Option(None, "--security-id", "-s", help="指定待回补历史的证券代码"),
...
@app.command("compare")
def compare_cmd(
    security_ids: list[str] = typer.Argument(..., help="待对比的ETF代码列表，例如 588200.SH 159915.SZ"),
):
...
def sync_holdings_cmd(
    security_ids: list[str] = typer.Option(None, "--security-id", "-s", help="指定待抓取持仓的证券代码"),
    top_n: int = typer.Option(20, "--top-n", "-n", help="按成交额自动选取前N只ETF"),
):
...
def screen_cmd(
    ...
    tag: str = typer.Option(None, "--tag", "-t", help="按穿透行业/风格标签筛选（如通信设备、半导体、电力设备、核心宽基/均衡）"),
    ...
    min_return_20d: float = typer.Option(None, "--min-return-20d", help="20日最小收益率（如 0.05 代表5%）"),
    max_drawdown_60d: float = typer.Option(None, "--max-drawdown-60d", help="60日最大回撤阈值（如 -0.15）"),
